...
## 概要
このリポジトリは、株式会社インゲージが提供する Re:lation API のドキュメントを整理し、参照しやすくしたものです。  
開発者がスムーズに API を利用できるよう、公式ドキュメントを Markdown 形式でまとめています。

## 免責事項
このリポジトリの内容は非公式であり、株式会社インゲージとは一切関係ありません。  
Re:lationの公式なAPI仕様や開発ドキュメントについては、以下の公式開発ガイドをご確認ください。

[公式開発ガイド](https://developer.ingage.jp/)

本リポジトリの内容の 正確性を保証するものではありません。  
API 仕様の変更により情報が古くなっている可能性もあります。  
ご利用の際は自己責任でお願いいたします。
...
- 
## フィードバック
本リポジトリの内容についてのフィードバックや修正提案を歓迎します。  
IssueやPull Requestを通じてご連絡ください。  
いただいたフィードバックをもとに、より正確で使いやすいドキュメントを目指します。
